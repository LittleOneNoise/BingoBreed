...
object ReproPlanner {
     ...
     *
     * [activeElementsByPaddock] = éléments de jauge actifs (ordinaux `hqf`) **par index d'enclos** (clé =
    ...
    /** Éléments d'enclos (ordinaux `hqf`) qui agissent sur la **sérénité** (pas une jauge de monture). */
    private val SERENITY_ELEMENTS = setOf(0, 1) // 0 = baffeur (baisse), 1 = caresseur (monte)

    /** Élément de jauge d'enclos (ordinal `hqf`) → type de jauge monture monté, ou null (sérénité/inconnu). */
